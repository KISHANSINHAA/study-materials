...
# PySpark program for Inner Join
...
spark = SparkSession.builder.appName("InnerJoin").master("local[*]").getOrCreate()

emp_df = spark.createDataFrame([(1, "Alice", 10), (2, "Bob", 20)], ["id", "name", "dept_id"])
dept_df = spark.createDataFrame([(10, "HR"), (20, "IT")], ["dept_id", "dept_name"])

inner_joined = emp_df.join(dept_df, on="dept_id", how="inner")
inner_joined.show()
