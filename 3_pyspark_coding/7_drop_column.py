...
# PySpark program to drop columns
...
spark = SparkSession.builder.appName("DropColumn").master("local[*]").getOrCreate()

data = [("Alice", 25, "NY", "Female")]
df = spark.createDataFrame(data, ["Name", "Age", "State", "Gender"])

# Drop State and Gender columns
reduced_df = df.drop("State", "Gender")
reduced_df.show()
