...
# PySpark program to rename columns in a DataFrame
...
spark = SparkSession.builder.appName("RenameColumns").master("local[*]").getOrCreate()

data = [("Alice", 25), ("Bob", 30)]
df = spark.createDataFrame(data, ["name_first", "age_years"])

# Rename column using withColumnRenamed
renamed_df = df.withColumnRenamed("name_first", "first_name") \
